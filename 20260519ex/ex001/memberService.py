...
사용자가
1'1.회원가입'을 선택하면 회원ID, 회원PW, 회원Email, 회원Phone 정보를 입력받아 회원가입 진행한다.
2'2.로그인'을 선책하면 회원ID, 회원PW를 입력받아 로그인 '성공' 또는 '실패'를 출력한다.   ->인증/인가
3'3.특정 회원 정보 출력'를 선택하면 회원ID와 회원PW를 입력받아 일치하는 회원 정보를 모두 출력한다.
4'4.모든 회원 정보 출력'를 선택하면 가입되어 있는 모든 회원 정보를 출력한다.
5'99.종료'를 선택하면 프로그램 종료 시킨다.
1-1 id 중복 체크
1-2 email형식이 아닐시 재시도
2-1 비밀번호 3회 이상 입력시 강제종료
